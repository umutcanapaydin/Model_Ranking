...
- **Principle:** if the project uses pydantic, both `pyproject.toml` lists it as dependency *and* `AGENTS.md` says "all schemas are pydantic ". One without the other is half a decision.
- **Origin:** S6 (D-019 + pyproject.toml lockstep).
- **Reusable artifact:** the dual presence pattern.
- **Risk if ignored:** agents see `pydantic` in deps and assume; or see "use pydantic" in docs and don't add it to deps.
...
- **Origin:** S18-bis — Error Handling Guide ingestion. We landed all three surfaces on 2026-05-21 / 2026-05-25.
...
## ADDENDUM (2026-06-01 — promoted to baseline 2026-06-02)

5 ACTIVE + 3 CANDIDATE seeds added at pipeline baseline, carried into unchanged.

### F.6 — Secret scanning is non-optional (ACTIVE; names gitleaks specifically)
- **Principle:** every commit runs a secret scanner (gitleaks). Pre-commit hook + CI gate. also fires via .claude/settings.json PreToolUse hook before writes to .env/*.env*.
...
- **Principle:** every task ends with structured Done Evidence: files changed, tests run + outcomes, assumptions, new ADRs, risks queued. PASS verdicts cite `file:line` evidence per acceptance criterion. makes this a permission-matrix §11 BLOCKING criterion if absent.
...
- **Principle:** agents shall not run `git reset --hard`, `git push --force`, `git checkout --` outside controlled recovery, or any destructive op without explicit user approval. Encoded in `permission-matrix.md` §5. candidate for PreToolUse hook promotion.
...
- **Principle:** when project touches auth / PII / payment surface, run SAST scan (Veracode, Semgrep, CodeQL) at Stage 4.1 Quality Gate. graduation criterion: first auth/PII milestone + G.12 PULLED-WEIGHT verdict.
...
- **Principle:** when subagent implements a NEW module, write tests first (citing REQ-IDs); subagent then implements until green. Only when K.8 contract is pre-locked. already operationalizes this via `/fix-issue` + `/fix-issue` skill split.
...
- **Principle:** when a milestone's domain demands a specific lens (Architect for cross-service, Migration-Specialist for schema), add a third profile under `subagent-profiles/`.: Code + Security are MANDATORY (D-005); third profile graduates only after ≥2 milestones of PULLED-WEIGHT.
- **Origin:** BMAD-METHOD pattern (43k+ stars suggests profiles); we reject that ceiling.
...
> Seeds harvested while adding Redis idempotency, durable delivery, drain, and backpressure to an async-callback adapter. Origin trail: Project-A `docs/process-log.md` S32–S33 + `docs/m12-retrospective.md`. Theme L (distributed correctness) is this milestone's main generalizable contribution; E.5 + C.10 extend existing themes.
...
- **Origin:** Project-A M12 (S33) — adding backpressure: the route claims the idempotency slot (`create_if_absent`) *before* dispatch, so a naive "queue full → 503" stranded the taskId as a permanent duplicate. Fix: on `DispatchQueueFull`, `delete(task_id)` to release the slot, then 503 — so the retry is a fresh accept.
...
- **Origin:** Project-A M12 (ADR D-043) — a pod that dies after delivering a callback but before acking it re-drives the job → a second, byte-equal, HMAC-signed callback for the same taskId. Documented as the contract rather than engineered away.
...
- **Origin:** Project-A M12 (B6 / D-044) — `RedisTaskStatusStore.create_if_absent` uses `SET … NX EX`; an 8-thread shared-server test asserts exactly one creator. A get-then-set version was explicitly rejected as a BLOCKING-class defect.
...
- **Origin:** Project-A M12 (S33, plan Wave 1.3) — `DISPATCH_QUEUE_MAX_DEPTH` + `DispatchQueueFull` → 503; off by default (0 = unbounded), ops sets it before scaling out.
...
- **Origin:** Project-A M11→M12 — M11 prod bring-up was death-by-a-thousand-crashes (one empty/invalid env per boot). M12 added `config_doctor.py`: validates everything at once, gated by `APP_ENV`, carrying the `REQUIRE_REDIS && !REDIS_URL ⇒ refuse boot` guard.
...
- **Origin:** Project-A M12 (S32) — a Wave-1 subagent died after landing the atomic-`SET NX` store but *before* writing the B1/B6 concurrency + restart tests. The code looked done; the proof was missing; K.7 caught it one stage later as BLOCKING. Cheaper at the dispatch boundary.
...
- **Origin:** Project-A M12 (S33) — target is 3.11+ (`datetime.UTC`), sandbox is 3.10 and `uv` couldn't fetch 3.12 (allowlisted network). A `sitecustomize.py` shimming `datetime.UTC` + a faithful `StrEnum` let the full suite run (446 passed / 1 skipped) and caught residual lint nits before the Mac round-trip.
...
- **Origin:** Project-A — first raised as a candidate at M10/S28 ("a green liveness probe says the server is up, not that it's running the code you think"; stale `make mock-qwen` *(example from a past project, not a GP target)* process scored 7/11). Graduated at S34 when a prod pod ran M11 code behind a green `/health` and it took three exec-and-grep checks (`/ready` 404, missing modules, ImportError) to prove it. Fix: `APP_BUILD` env → `/health` returns `{status, version, build}`.
...
- **Origin:** Project-A S34 — an M12 feature merge overwrote DevOps's `Dockerfile` customizations on `master`; CI built from the clobbered file (or didn't rebuild the expected image), so the deployed pod stayed on M11 code. Cost: a deploy that looked done but wasn't, plus a DevOps round-trip to restore their changes.
...
## RATIFIED — two-project bootstrap + prod bring-up feedback (2026-06-19)

> Ratified for by a 7-role blind council (Senior SW, Senior Data, PM, QM, Security, DevOps, Cloud Architect) + the product owner. Sources: Project-B bootstrap (FB-1..FB-5) and Project-A S35 prod bring-up (L.8, L.9, K.11, E.6). Through-line: **documented discipline is not self-enforcing -> make it an executable gate, at Stage 0 and at go-live.** Append-only; supersede, don't edit.
...
- **Origin:** Project-B S1/S2 -- the starter even shipped a `{status:ok}` health test contradicting the design doc's L.7 day-1 claim; the gap was in the starter, not just the init agent.
...
- **Origin:** Project-B S1 (universal D-006/D-007 collided with the project's own D-006+; reconciled by moving process ADRs out of the project D-band).
...
- **Origin:** Project-B -- the wrapped NewAPI gateway is AGPL-3.0 (network copyleft + Section 7 attribution); surfaced only via deep code analysis (`docs/research/newapi-legal-note.md`).
...
- **Origin:** Project-A S35 -- Dify's Huawei MaaS provider listed DeepSeek/Qwen with a valid token, but every call returned `ModelArts.81009 "Invalid model"` (nothing deployed); only a Test Run exposed it.
...
- **Origin:** Project-A S35 (and again from M11) -- the `llm-base` chart injects `secrets.environment` but NOT `configMap.data`; an AppCode set under `configMap.data` was `{}` in the pod. Caught only by reading the env from inside the pod.
...
- **Origin:** Project-A S35 -- with the model down, a `/` call returned `200 accepted` and Dify's run log showed the run attributed to `Project-A-adapter`, failing only at the model node; that one line proved the whole pipe except the model.
...
- **Origin:** Project-A S35 -- configured 3 Dify workflows (model repoint + publish) + ran a live Test Run + read the run log entirely through the browser; the user did the credential/cluster steps.
...
- **What changed:**§15.8 listed council planning as PULLED-WEIGHT but N=1, deferring promotion until a 2nd independent payoff. Project-A ran two more blind parallel councils planning + auth/D-029, and this very cut was decided by a 7-role blind council -- the 2nd and 3rd payoff. Graduated to a:** standing OPTIONAL Stage-1 variant** (kept optional for anti-bloat).
...
## RATIFIED — cross-project harvest (2026-06-26)

> Ratified for by a 13-seat blind council (6 core + skeptic voting all 68 candidates; 7 domain seats voting their clusters) + the product owner, splits S1-S4 settled by the owner. Sources: 7 projects -- Project-I (expense claims, Node/Vue), Project-H (decoration marketing site, React), Project-D (Next.js AI-ops), Project-F (multi-tenant AI CS), Project-E (Project-E), Project-B (NewAPI Go gateway), Project-C (LLM gateway, Go). Budget §5.5: ~15 adopts, ≤3 BLOCKING gates (2 spent, 1 reserve). Through-line: **the largest new gap is agent-native/LLM-ops + gateway, but most of that evidence is one ecosystem, so it is chartered as candidate, not adopted wholesale.** Append-only; supersede, don't edit.

### — Web/API security baseline: no plaintext creds / no default-admin password. (GATE; ACTIVE)
...
- **Origin:** Project-I F1 (plaintext passwords, catastrophic) + Project-C F12 (hardcoded default admin password, catastrophic) -- two independent projects re-derived GP's inherited security gate the hard way.
...
### — No "done" without a citing test per acceptance criterion; red-test the symptom first. (GATE; ACTIVE)
...
- **Origin:** Project-I F11, Project-B F1 (test-plan doc) -- convergent across ecosystems + validates GP's E.4/E.2.
- **Reusable artifact:** the line in the Quality Gate (`docs/closure-checklist.md` §B.1) + AGENTS.md §3.3 + the per-wave `subagent-profiles/Tester.md`.
...
### — Restructure the review loop: per-wave Code-Reviewer + Tester + per-agent dev-test loop; Security review -> milestone closure (BLOCKING before deploy). (TEMPLATE-CHANGE; ACTIVE)
...
- **Origin:** owner proposal 2026-06-26 + convergent test evidence (Project-C F9 race). Replaces the per-wave Duo (Code + Security) with per-wave (Code + Tester) + dev-test loop; Security joins Quality at Stage 4.
- **Reusable artifact:** revised Stage 2/3/4 in [`METHODOLOGY.md`](METHODOLOGY.md); `subagent-profiles/Tester.md`; the closure Security step (`docs/closure-checklist.md` §B.2a); the HIGH-risk security trigger in `permission-matrix.md`.
...
### — One canonical mock per integration, built before integration code, + a contract test. (TEMPLATE; ACTIVE)
...
- **Origin:** Project-F F6+F8 (mock divergence), Project-B F6, Project-C -- 3 gateways; validates GP's K.1 / J.4.
- **Reusable artifact:** the canonical-mock convention in [`METHODOLOGY.md`](METHODOLOGY.md) §7 (testing) + the closure check in `docs/closure-checklist.md` §A; tests drive the canonical fake, never bespoke per-test stubs.
...
### — Server-side authz on every mutating route; client checks are UI sugar. (GUARDRAIL; ACTIVE)
...
### — CORS allowlist in prod; never allow-all + credentials. (GUARDRAIL; ACTIVE)
...
- **Reusable artifact:** `docs/security-baseline.md` item 3; `.agents/rules/practices.md` guardrails.
...
### — Validate security-critical config at startup; fail the prod process. (GUARDRAIL; ACTIVE)
...
- **Origin:** Project-F F9 (startup config validation) + Project-C F15 (fail loud on parse error, never default to zero).
...
### — Encrypt credentials/PII at rest with a rotation-friendly multi-key chain. (GUARDRAIL; ACTIVE)
...
### + — Control class decides fail direction: auth/safety fail CLOSED, fairness fails OPEN. (GUARDRAIL, paired; ACTIVE)
...
- **Origin:** Project-F F2 (fail-closed safety) + Project-B F3 (fail-open rate-limit) -- the one genuinely cross-ecosystem agent-native pair.
...
### + — Agent least-privilege tool allowlist + human-confirm on all writes (CI and runtime). (GUARDRAIL; ACTIVE)
...
- **Origin:** GP's own issue-agent.yml + Project-F F2 (per-agent tool allowlist + human-confirm) -- multi-project + GP.
- **Reusable artifact:** the allowlist + human-confirm rule in `permission-matrix.md` §5/§8 + `.agents/rules/practices.md`; extends (Layer-2 issue agent).
...
### + — No destructive ops; destructive defaults OFF. (GUARDRAIL; ACTIVE)
...
- **Origin:** Project-H F3 (surgical revert; destructive default-on reseed).
- **Reusable artifact:** `permission-matrix.md` §5 (reseed default-off row + safety guardrails); the catastrophe-class list.
...
### — Build guardrails. (GUARDRAILS; ACTIVE)
- **Principle:** read per-environment config at runtime, never bake it at build time (generalizes L.9 to build-time bake, e.g. Next.js); every dependency used is saved to the manifest in the same edit (extends C.6 to JS/npm); pin the runtime/toolchain version in CI (`.nvmrc`/`engines`) and test the build on the target; run the race detector (`-race`) on concurrent packages as a recommended CI step (a `// BUG:` touching shared state blocks release).
- **Origin:** Project-H F2 (Vercel build break), Project-H F5 (toolchain pin), Project-C F9 (data race).
...
- **Tradeoff / cost of adoption:** is recommended-not-gate (evidence is Go-only); the rest are near-free hygiene.

### — Confirm / fold (doc). (ACTIVE doc updates)
- **Principle:** write design notes + a gap-analysis (what exists vs what to build) before the first line of code -- keep it light (ceremony risk); commit per-repo rules to `.agents/rules/` + treat AGENTS.md §8 as a PROCESS routing index with lazy doc-loading (token economy) -- independently re-derives GP's own design; confirms L.7 (version-stamped /health); commit `.gitignore` first.
- **Origin:** Project-B F1 (design+gap-analysis); Project-F F3/F4 (.agents/rules + routing index); Project-I F5 + both GP-v2.2 runs; Project-H F1.
- **Reusable artifact:** Stage-1 gap-analysis line + START_HERE routing-index note ([`METHODOLOGY.md`](METHODOLOGY.md), `START_HERE.md`); these validate existing GP design (raise confidence, no new mechanism).
- **Risk if ignored:** code before a clear build/reuse picture; AGENTS.md bloats; untracked files leak into the first commit.
- **Tradeoff / cost of adoption:** keep to a few lines so it doesn't become a heavyweight design phase for small milestones.

### ★ CANDIDATE sub-block — Agent-Native / LLM-Ops theme (NOT active; promote on a 2nd independent ecosystem)

> Chartered as a **container**, not adopted. The evidence is mostly ONE ecosystem -- Project-F/Project-E/Project-D (Project-F, Project-E, Project-D) + the Project-C ≈ NewAPI gateway family (Project-C, Project-B). Per the council's weighting rule, down-weight intra-ecosystem agreement; promote a seed only when it also appears OUTSIDE one ecosystem or re-derives GP's own design. The four that already cleared that bar are ACTIVE above; the rest wait here. Full text per candidate in the candidate block below. Append-only; do NOT promote without a 2nd independent ecosystem + council/owner sign-off.

- **CANDIDATE:** LLM providers in a DB registry (runtime-swappable, probe-verified), not hardcoded env keys. *Sources: Project-F F1, Project-C F5.*
- **CANDIDATE:** every LLM step has a deterministic fallback, tagged with `source` (llm vs fallback).
- **CANDIDATE:** force a machine-readable LLM output contract (fenced JSON schema) + a neutral default on parse failure; never parse prose.
- **CANDIDATE:** dual-grounding: LLM for qualitative judgment, deterministic queries for every checkable number.
- **CANDIDATE:** number provenance: label real/simulated/estimate, log score↔outcome from day 1, no silent imputation.
- **CANDIDATE:** instrument every LLM call with tracing (prompt/tokens/latency/tools) from the first call. *Project-F F10.*
- **CANDIDATE:** add new capability as a dedicated single-purpose agent, not a new router skill on a proven agent.
- **CANDIDATE:** separate ephemeral AI/session state from durable business/workflow state with a one-way escalation link. *Project-F F14.*
- **CANDIDATE:** rule-based retrieval over vectors on small corpora; split into two corpora when the join key is broken.
- **CANDIDATE:** MCP tool servers on stateless HTTP (curl-testable) + a lazy process-lifetime connection pool, never closed per request. *Project-F F5.*
- **CANDIDATE:** streaming token rate-limiting: pre-reserve `max_tokens` upfront, post-adjust by actual after the stream (Project-C: same for two-phase billing). *Project-B F2, Project-C F2.*
- **CANDIDATE:** structured 429 + `Retry-After`; pick sliding-window (RPM) vs fixed-window (TPM) by precision need. *Project-B F9/F11.*
- **CANDIDATE:** multi-DB ORM abstraction encoded as written rules before the first cross-DB bug (driver from DSN at runtime). *Project-B F5, Project-C F4.*
- **CANDIDATE:** split the read-only data plane from the write/action plane; integrate as complementary tools.
- **CANDIDATE:** prove "no side-effects" safety constraints by tracing the code path + a negative grep, never by assumption (extends E.5/E.6).
- **CANDIDATE:** circuit-breaker: auto-disable a failing upstream on a rolling success-rate, tri-state status (enabled / manually-disabled / auto-disabled) so automation never overrides an operator. *Project-C F6.*
- **CANDIDATE:** classify upstream failures by stable signals (HTTP status, structured type/code), never by matching error-message substrings. *Project-C F8.*
- **CANDIDATE:** one minimal lifecycle interface per external backend (adaptor pattern) → adding a provider is a localized copy-one-file task (extends K.1). *Project-C F1.*


---

## RATIFIED (2026-07-03) — Project-B field harvest + owner directives (11-seat blind council)

> First field run of GP itself (Project-B, M0–M4, 171 tests). All six v3-adopt validations CONFIRMED, five sharpened, none contradicted. **0 new gates** (reserve preserved). New meta-rule (council-design §5.6): validates-GP evidence from projects run ON GP cannot alone escalate anything past template weight.

- **ACTIVE, guardrail+template:** discipline is executable, never documentary: Stage-0 / wave close / milestone close each run an executable check or a committed checklist file (`docs/wave-checklist.template.md`); every ✅ cites a fresh, wave-scoped evidence referent; skipped/waived checks are ledgered, never silent. *Project-B F1 (N=3) + owner .*
- **ACTIVE, guardrail:** day-0 license/commercial-use review of any wrapped/forked OSS engine (AGPL/GPL/SSPL ⇒ wrap-not-fork + sign-off) BEFORE architecture depends on it. *Project-B F2 (catastrophic, averted).*
- **ACTIVE, template:** consumption posture named explicitly: wrap | fork | port + change triggers; the wrapper never touches the wrapped system's datastore. *Project-B F3.*
- **ACTIVE, template:** Tester fault-injection: break → confirm RED → revert in place, verify byte-identical (md5); a fault that STAYS GREEN is the finding → mandatory new test. Scoped to load-bearing/HIGH-risk slices. *Project-B F5 (N≥4).*
- **ACTIVE, guardrail:** "built ≠ wired": acceptance criteria phrased end-to-end on the live path; the citing test enters through the live entrypoint; review asks "is this control reachable from the request path?" *Project-B F6.*
- **ACTIVE, guardrail:** every security invariant has its NEGATIVE test (fails when the invariant is removed), listed per milestone with citations; checked at the BLOCKING security close. *Project-B F7.*
- **ACTIVE, doc:** idempotency test = same key, DIFFERENT payload, assert first-write-wins. *Project-B F8.*
- **ACTIVE, guardrail — domain-scoped: projects handling money:** money is integer minor units + currency end-to-end; the Money type rejects float; round half-up exactly once at the boundary. *Project-B F12.*
- **ACTIVE, template — amends via P-005:** risk-tiered review depth: LOW/MED wave → ONE combined reviewer; HIGH (auth/payment/crypto/migration/distributed-correctness; auto-escalated on authz/secrets/crypto/input-parsing/egress diffs) → Code + Tester + pulled-forward security-on-slice. Escaped-blocker tripwire reverts to full review. *Project-B F14 (measured ~11→~5 reviewer runs).*
- **ACTIVE, template:** every retrospective answers the previous retro's carried question and poses one; a rule proposed twice but never built is BUILT or DROPPED (verdict buckets + retired count already active via G.12/D.4). *Project-B F18.*
- **ACTIVE, guardrail+template:** living EXPERIENCE.md (`docs/EXPERIENCE.template.md`): appended at every milestone closure; the quarterly handover BLOCKS without a dated entry for the latest closed milestone; no secrets/PII; findings cite evidence. *Owner .*
- **Sharpened (existing seeds, same IDs):** + one live contract test vs a pinned real instance (self-skip is LOUD, counted at closure) + pre-freeze spike *(F4)* · + TESTED disable switch + fail-direction table at security close *(F10)* · + deny-service enforcement OFF-in-code/ON-in-prod-profile + boot preflight *(F13)* · + wave-close gates on plan-tagged passes *(F15)* · /C.6 + hermetic gate recipe *(F16)* · + revert in place, NEVER checkout/restore on uncommitted work *(F17)*.
- **DEFERRED:**Redaction assert-absence-in-sink — promote fast on a 2nd ecosystem · generator principle "customer progress view ≠ internal truth tracker" rides as doc .:** Agent-Native theme unchanged** — Project-B is gateway-family (wraps One API), NOT the awaited 2nd independent ecosystem.


---

## RATIFIED (2026-07-03) — external research harvest + owner autonomy directive (2-phase council: 6 core → 3 domain, chair decided under owner delegation)

> Source: the 9-doc Agentic-Engineering research curriculum (`research/agentic-engineering-curriculum/`) — the first fully EXTERNAL evidence class — + owner directive (PRD → sign → autonomous run → owner reviews results + git). **0 new gates.** Central rule, converged on blind by 9/9 seats: **anything feeding trust/promotion/gates is computed from git/CI/hooks against protected refs; agent-asserted content is context, never a gate input.**

- **NORTH-STAR CANDIDATE — NOT ACTIVE; owner decision 2026-07-03: "we are not ready — I review every wave and milestone, run the tests/checks, and make the commits"; A0 is the only operating mode, activation only by future owner-initiated ADR:** autonomy designed as a LADDER (A0/A1/A2, `docs/autonomy-protocol.md`): promotion = 2 consecutive mechanically-clean milestones; demotion automatic, non-waivable, asymmetric; ⛔ zones glob-detected + human line-by-line at every level; per-plan scope grants replace per-write confirms; assumption ledger replaces mid-loop questions (⛔/criteria-meaning = HALT); blocked = halt-and-notify, never improvise; backpressure pauses the loop when closure packs sit unreviewed. * + trust-ladder/harness-loop literature.*
- **ACTIVE, template:** every closure generates the OWNER REVIEW PACK — an AID to the owner's wave/milestone review, never a replacement for it — (`docs/closure-report.template.md`), derived from raw referents, 2-page cap, prose architecture-delta BLOCKING ("can't explain it → doesn't ship"); replaces the separate §B walkthrough output + note.txt milestone summary; absorbs the agent-authorship commit trailer. * + comprehension-debt literature.*
- **ACTIVE, template:** trust telemetry per task type at every closure: post-closure fix rate (path overlap vs protected closure tag), churn, reverts, findings (security ×2) — script-computed into cost-log. Purpose TODAY: honest quality signal for the owner + the multi-version track record the north star would someday need. *Progressive-trust demotion rule; Apiiro 10× trigger.*
- **ACTIVE, doc+profile:** context economy: one task/session; fresh subagents per wave; exploration via the read-only **Explorer** profile (≤2k-token summary; NEW `subagent-profiles/Explorer.md`); compaction anchored to wave close, preserving open findings verbatim + re-injecting security invariants; token budget as a live circuit breaker; continuity is FILES, not sessions. *Anthropic context evals (−84% tokens; +90.2% isolation — construct caveat recorded).*
- **ACTIVE, template — one home per check:** AI-generation review smells: duplication-vs-reuse, drive-by edits, swallowed exceptions (Code-Reviewer); mirror-implementation + weakened/deleted-to-green tests (Tester, BLOCKING at HIGH); ~≤400-line wave diff WARN (wave checklist). *GitClear 8×, CodeRabbit 1.7×, DORA −7.2%.*
- **ACTIVE, doc + mechanical backstop:** declared L0 spike lane: `spike-*` exempt from gates EXCEPT secrets scanning; NEVER merged (branch-guard + closure check + delete-at-close); productionize = rebuild. *Mode-drift incidents (Replit, Tea).*
- **ACTIVE, doc — scoped MED/HIGH:** plans name ONE alternative + trade-offs; rejection anchored to concrete absence (mechanically checkable at A2). LOW-tier exempt.
- **External corroboration recorded (B1–B7):** executable-discipline (first independent confirm), the security gates (Veracode 45%, no scale improvement), slopsquat defense (USENIX 19.7%), AGENTS.md diet, fresh-eyes isolation (with the Skeptic's construct caveat on +90.2%), red-test-first, ⛔ zones.
- **Standing obligation:** the retro MUST report the retired count of 's own additions — zero retirements = the accretion-cap promise was decoration; treat the next autonomy escalation accordingly.


---

## RATIFIED (2026-07-05) —: A0.5 milestone-cadence owner review (7-seat council, chair-delegated)

- **ACTIVE — PROVISIONAL, guardrail+template:** operating mode **A0.5**: waves close agent-side; the owner's deep review, personal test runs, and commits happen at every milestone boundary (capped ~4–6 waves / ~2k net lines; 60–90 min session; per-wave table + "decisions on your behalf" in the closure report). Owner checkpoint commits per wave (`wip: NOT reviewed`; agents never run git). Escalate-NOW list halts mid-wave; assumption ledger active; reviewer countersigns checklist rows; semantic-security items added to the agent pass. **Bright line: agent commit on main = A1 = explicit owner ADR.Tripwire:** escaped blocker an owner wave-pass would plausibly have caught → auto-fallback to wave cadence. * + Project-B field evidence (single project — hence provisional); Skeptic dissent recorded.*
- **Increment-6 harvest note:** final Project-B EXPERIENCE = md5-identical to the interim → obligation CLOSED, zero new findings; the md5 duplicate check is PULLED-WEIGHT (two catches).
...
## RATIFIED (2026-07-17) —: Stage 5 Maintenance Loop (5-seat council)

- **ACTIVE, template+guardrail:** post-deploy fix discipline: fix waves = normal waves
 ...
 debt, >1/month alarm. * + Project-B post-prod (verbal, ~5 fixes; written harvest owed).*
...
- **RETIRED by this cut:** the standalone, memory-based EXPERIENCE harvest session (3 md5-identical
 ...
 reports. **Debt line:** the retirement count is STILL owed to the first + project retro.


---

## RATIFIED (2026-07-27) —: the first post-prod dataset (5-seat council)
...
> what we imagined and blind to the shape of the world we deployed into.** points the
> machinery outward. 0 new gates; net new package files: 0. Skeptic's dissent recorded: two cuts
> in one week is not precedent.

- **ACTIVE, guardrail:** ~~`make check-templates`~~(SHIPPED templates instantiate the <!-- **REMOVED at the control screen (2026-08-12) — on `docs/watchlist.md`, returns at 2 recurrences.** It needs a deployed URL and a live environment that neither this package nor a fresh project has, so in five versions nobody could write down how to break it and it was never once shown to catch anything. --> <!-- REMOVED at the control screen 2026-08-12 -- see `docs/watchlist.md`, returns after 2 recurrences. It needs a deployed URL and a live environment neither this package nor a fresh project has, so in five versions nobody could write down how to break it and it was never once shown to catch anything. Left visible on purpose: a control that quietly vanishes is how a checklist becomes folklore. -->
 parser) + ~~`make cold-start`~~(zero persisted state → serve-ready/honest not-ready); CI at <!-- **REMOVED at the control screen (2026-08-12) — on `docs/watchlist.md`, returns at 2 recurrences.** It needs a deployed URL and a live environment that neither this package nor a fresh project has, so in five versions nobody could write down how to break it and it was never once shown to catch anything. --> <!-- REMOVED at the control screen 2026-08-12 -- see `docs/watchlist.md`, returns after 2 recurrences. It needs a deployed URL and a live environment neither this package nor a fresh project has, so in five versions nobody could write down how to break it and it was never once shown to catch anything. Left visible on purpose: a control that quietly vanishes is how a checklist becomes folklore. -->
 merge+release; no default creds in the boot path. *FIX-01/02: two total outages.*
- **ACTIVE, template:** human-path: a non-author, shipped docs only, completes the
 primary journey (credentials instance mandated). *FIX-04.*
- **ACTIVE, guardrail:** producer enumeration on hardened invariants: checklist row +
 required verdict section + citing test per producer; security signs auth-class. *FIX-03.*
- **ACTIVE narrow, doc:** never parse a bounded prefix; diagnostics print revision
 stamps. Broad tooling-rigor doctrine deferred. *FIX-06.*
- **ACTIVE, guardrail:** ready≠alive (stack-conditional) + diagnosable fail-closed with
 the disclosure channel in the criterion (logs + authed diag only). *FIX-02/05.*
- **SPLIT:** boundary-grep line ("touches build/CI/k8s: NO", machine-verified) ACTIVE;
 full patch-package format CANDIDATE (one partner). *§4 of the harvest.*
- **ACTIVE, edit:** cadence binds to ARTIFACTS: every outward deliverable = a wave
 close. *FIX-07.*
- **ACTIVE, template — default-expected, not mandatory:** black-box journey tester
 (~~`make journey URL=…`~~, stdlib, shipped in-package, authored during build; QM bar: cold entry · <!-- **REMOVED at the control screen (2026-08-12) — on `docs/watchlist.md`, returns at 2 recurrences.** It needs a deployed URL and a live environment that neither this package nor a fresh project has, so in five versions nobody could write down how to break it and it was never once shown to catch anything. --> <!-- REMOVED at the control screen 2026-08-12 -- see `docs/watchlist.md`, returns after 2 recurrences. It needs a deployed URL and a live environment neither this package nor a fresh project has, so in five versions nobody could write down how to break it and it was never once shown to catch anything. Left visible on purpose: a control that quietly vanishes is how a checklist becomes folklore. -->
...
- **ACTIVE, doc:** every boot prerequisite: in the image OR a named-owned provisioning
