...
// MARK: - M12-W3 — the budget a reader can actually choose

final class BudgetOptionTests: XCTestCase {

    private func decode(_ json: String) throws -> [BudgetOption] {
        try JSONDecoder().decode(BudgetList.self, from: Data(json.utf8)).budgets
    }

    /// Copied from what `/v1/budgets` actually returns, not written from the Swift struct — the
    /// mistake `Models.swift` opens by warning about and which this file made once already.
    private let served = #"""
    {"api_version": "v1",
     "blend": {"note": "blended $/1M = input*0.75 + output*0.25",
               "input_weight": 0.75, "output_weight": 0.25},
     "budgets": [{"id": "low", "blended_cap_per_m": 2.0},
                 {"id": "medium", "blended_cap_per_m": 8.0},
                 {"id": "unlimited", "blended_cap_per_m": null}]}
    """#

    func testTheServedPayloadDecodes() throws {
        let budgets = try decode(served)

        XCTAssertEqual(budgets.map(\.id), ["low", "medium", "unlimited"])
        XCTAssertEqual(budgets.first?.blendedCapPerM, 2.0)
    }

    /// `null` is the ABSENCE of a cap. Turning it into a number here would reintroduce the
    /// sentinel D-134 deliberately refused to publish, one layer up.
    func testUnlimitedHasNoCapRatherThanAVeryLargeOne() throws {
        let unlimited = try XCTUnwrap(try decode(served).first { $0.id == "unlimited" })

        XCTAssertNil(unlimited.blendedCapPerM)
        XCTAssertNil(unlimited.capLabel, "a cap label was invented for a budget that has no cap")
    }

    /// The CFO finding, applied to this control before it ships rather than after: the engine's
    /// ids are a vocabulary, and this product had been asking people to learn its vocabularies.
    func testTheButtonSaysWhatItMeansRatherThanTheEnginesWord() throws {
        let budgets = try decode(served)

        XCTAssertEqual(budgets.map(\.title), ["Cheaper", "Mid-priced", "Any price"])
        for option in budgets {
            XCTAssertFalse(option.title.lowercased().contains(option.id),
                           "`\(option.title)` is just the engine's id with a capital letter")
        }
    }

    func testTheCapIsShownBesideTheNameBecauseTheNameAloneIsASecondVocabulary() throws {
        let low = try XCTUnwrap(try decode(served).first { $0.id == "low" })

        XCTAssertEqual(low.capLabel, "under $2/1M")
    }

    func testACapIsNotPrintedWithTrailingZeroNoise() throws {
        // `$2/1M`, not `$2.000000/1M`. The reader is a CFO; the number has to look like money.
        let medium = try XCTUnwrap(try decode(served).first { $0.id == "medium" })

        XCTAssertEqual(medium.capLabel, "under $8/1M")
    }

    func testAnUnknownBudgetStillGetsAReadableLabelRatherThanCrashing() throws {
        // The engine may add a budget before the app knows about it. An unknown id is shown, not
        // hidden: hiding it would silently drop a choice the engine offers.
        let budgets = try decode(#"{"budgets": [{"id": "enterprise", "blended_cap_per_m": 40.0}]}"#)

        XCTAssertEqual(budgets.first?.title, "Enterprise")
        XCTAssertEqual(budgets.first?.capLabel, "under $40/1M")
    }
}
