...
# Fixpack — {X.Y.Z+1} (Stage 6 Maintenance Loop; THE DEPLOY GATE)

> **Copy to `docs/fixpack-{N}.md`. Deploy is BLOCKED until every row is complete.** One page.
> A fix wave is a WAVE — `/close-wave`, the normal wave checklist and both fresh-eyes reviews apply
> (reference, not restated here). This doc adds only the fix-specific evidence and the owner's out-of-sandbox gate.
...
- Pack ID: `fixpack-{N}` · Version: `{X.Y.Z+1}` (stamped into `/health` build)
...
## Deploy & watch (the Stage 5.2 gates + these)

- [ ] Stage 5.2 gates passed (new SHA live via /health; restart ≠ rebuild)
- [ ] **Fix probe:** each previously-broken behavior exercised in prod — new behavior confirmed
- [ ] `make journey URL=<deployed>` re-run green after the pack lands (the same script as the release's)
- [ ] Watch window `<30–60 min; longer for HIGH>` clean — errors/logs flat → pack CLOSED
- [ ] If the journey also runs on a schedule: that schedule is still active post-pack (named on-call owner, flake/mute policy; result lands in this watch record)
...
## Capture coupling (deploy condition)
