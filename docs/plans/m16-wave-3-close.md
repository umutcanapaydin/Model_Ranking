...
|---|---|---|---|
| 1 | Risk tier recorded for this wave in the plan | `docs/plans/m16-plan.md` §2 W3 "(risk: **MED**)"; the branch plan reached main with PR #3 and was deleted right after (DevFlow: a plan never stays on the default branch) | ✅ |
...
| 9 | Skipped/waived ledger + run summary | `gates run: make check (pytest 1067/15, Swift 268, conformance 14 PASS, client-decls, check-records, wave-check-all) · make falsify deps slopsquat PASS · make secrets FAILED on an UNTRACKED file in the owner's working tree (an HTML file at the repo root, 2 generic-api-key findings; not in the branch, not written by this wave), escalated to the owner; the branch itself: gitleaks detect --log-opts origin/main..HEAD, 22 commits, no leaks · gates SKIPPED: none · outcome: draft PR #3, owner merges` | ✅ |
