...
> **STATUS: CLOSED 2026-08-17.** The fresh-eyes review returned BLOCKING; the re-review closed both
> and the wave with a MINOR verdict. `make check` exit 0, 313 passed / 12 skipped, fault injection
> 8/8. W-018 RESOLVED.
>
> The two BLOCKING findings were the same lesson in different clothes:
>
...
## The question this wave could not answer by testing, and the answer it got

The wave record claimed the hand-mirror class was closed. It is not, and the reviewer's correction
is worth more than the claim was:

> **No black-box output test can distinguish a correct mirror from a derivation** — asking one to is
> asking a test to see intent. What the derived test buys is detection of the mirror's **rot**, not
> its **existence**: `expected` comes from `fields(Recommendation)`, so a mirror goes red the instant
> the engine gains a field. *A mirror cannot silently stay correct, though it can silently exist.*

Measured, not argued: a COMPLETE hand mirror restored into the adapter leaves all 310 tests green.
A test *could* catch it by asserting the CALL rather than the output — monkeypatch the serializer to
a sentinel — and both seats preferred the sentence to the test, because a call-assertion couples the
