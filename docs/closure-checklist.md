...
- [ ] **v3.5 (V3C-99):** `make check-templates` green — every SHIPPED config template instantiates the settings parser (CI run cited)
- [ ] **v3.5 (V3C-99):** `make cold-start` green — boot against ZERO persisted state reaches serve-ready or an honest not-ready (CI run cited)
...
- [ ] **v3.5 (V3C-106):** `make journey URL=<deployed>` green (default-expected; skip recorded in the closure report) — cold entry · credential lifecycle · paying-customer round trip asserting CONTENT · one cross-wave sequence
