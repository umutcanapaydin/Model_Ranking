# Autonomy Protocol & Ladder (**active mode: A0.5**; A1/A2 = NORTH STAR, NOT ACTIVE)

> ## ⛔ STATUS (updated by owner directive, 2026-07-05 — binding)
> **The active operating mode is A0.5 — "milestone-cadence owner review" (PROVISIONAL,
> tripwire-protected).** Waves close agent-side
...
> **Superseded:** the git clause of A0.5 is replaced by one rule — the agent opens drafts, a human marks them ready and merges. There is no checkpoint commit and no dedicated
make target for one; the agent commits normally on its own branch. *(historical: ~30-second labeled checkpoint commit per wave, `wip: NOT reviewed` — commit ≠ approval).
...
> Ratified 2026-07-03; external basis: progressive-trust
...
- **Demotion (auto, non-waivable, asymmetric):** ONE tripwire event drops A2→A1 (or A1→A0 on a
 ⛔-zone breach): an escaped blocker on a tiered-down wave (tripwire), a post-closure fix
...
1. **Write confirmation ** → a **per-plan scope grant**: the signed milestone plan
 ...
 **path/glob patterns declared in the plan** (`security-globs`), never by agent self-classification.
 Any diff touching a ⛔ glob forces the wave to HIGH tier (auto-escalation) and the slice
...
- **Blocked at A1/A2 = halt-and-notify, never improvise:** on a BLOCKING security verdict, a
 tripwire, or non-assumable missing info, the controller (1) commits on its branch — branch pushed, draft
...
*Weight: guardrail + template. Owner directive; safety shape per the council. Supersedes
nothing — extends and the permission matrix; P-006 records the adoption.*
