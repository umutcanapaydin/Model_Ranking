...
| REQ-APP-004 | The app degrades honestly: engine unreachable, 503, an empty answer and a slow response each produce a stated condition — never a blank screen and never an endless spinner. | **MET, with one case unreachable.** Verified LIVE for engine-unreachable: the engine was stopped and the app stated the condition, the remedy and a retry.  and  are new named cases bounded by a 10-second timeout ( waits 60, which is the endless spinner the plan names) and are gated structurally, not exercised. **503 cannot currently be reached at all:** the startup probe refuses to boot on an unbuilt artifact, so 's 503 branch has no live path — recorded as W-039 rather than claimed as covered. |
