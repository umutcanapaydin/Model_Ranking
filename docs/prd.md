...
| REQ-FLR-002 | The detail screen shows that floor as the line below which the product does not recommend, in both languages, composed in the Engine from the served fact. | **M16-W2, not built.** The field it reads exists (REQ-FLR-001); the screen is the next wave, and W-112 stays open until it ships. |
...
| REQ-PRC-002 | Wherever the app shows a price for `search` or `search_factuality`, it also says that a search call is not in it. | **M16-W2, not built.** W-119 stays open until it ships. |
