...
| REQ-SCR-002 | The conversion is per surface and strictly monotonic: it never reorders a ranking. | **M14-W4.** Cited by `ScoresTests.swift::testTheConversionNeverReordersOnAnyAnchor` (anchors 1100–1600 every 12.5, ±400 Elo at 0.1; D-162 made the anchor a derived floor). |
| REQ-SCR-003 | *(amended by D-162, #15)* An Elo surface's anchor is the floor it recommends from, derived from the served board, so 50 out of 100 is "at the bar"; never the board's maximum. | **M14-W4; amended #15.** `/v1/categories` serves `score_anchor` = `min_quality` on Elo surfaces (D-162). Cited by `tests/unit/test_floor_served.py::test_every_elo_surface_anchors_its_score_at_its_served_floor`, `test_the_anchor_moves_with_the_board`, `test_with_no_artifact_no_anchor_is_invented`, and the row pins in `test_ios_client_contract.py` (review M-4 mutant). |
...
| REQ-ID | Criterion | Status |
|---|---|---|
| REQ-FLR-001 | `/v1/categories` publishes each surface's `min_quality` (since D-159 derived from the served board by `src/app/workflows/floors.py`; `null` with no artifact) on that surface's own scale, as its own field. On an Elo surface `score_anchor` is the same number since D-162 (#15): the anchor follows the floor, never the other way round. | **M16-W1 (engine half); since M17-W1 the floor is derived (D-159).** `src/app/adapter/main.py::categories`. Cited by `tests/unit/test_uncertainty_contract.py::test_every_surface_publishes_the_floor_it_recommends_from` (every served floor equals `derived_floor` on the served artifact) and `tests/unit/test_floor_served.py` (the floor moves with its board, and since D-162 the anchor with it; `null` with no artifact; a floor moved by unranked rows is a served change). Before M17-W1 it moved one surface's FLOOR and not its anchor and was shown RED on three mutants: the field dropped, the field served from `score_anchor`, and the floor moved. |
