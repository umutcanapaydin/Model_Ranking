...
| REQ-RUN-001 | The product has been operated by a person against a running engine, and what was asked and what came back is written down — including anything that looked wrong. | **M11-W3: the SESSION is ready, the WALK is the owner's.** `scripts/simulator_session.sh` starts the engine, verifies its build stamp and evidence state, builds, installs and launches the app on a booted simulator in one command. Proven end to end by running it: `com.ilgar.modelranking` is installed and launched. The criterion is NOT met until a person has walked it and written the result down. |
