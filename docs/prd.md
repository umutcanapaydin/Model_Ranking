...
| REQ-IOS-002 | The router's boundary is proven IN SWIFT: no path yields an id outside the nine, every tier can be absent without blocking the screen, and an unmeasured question reaches the surface flagged as unmeasured. | **M11-W2 DONE.** `ios/EngineTests/RouterBoundaryTests.swift`. Required opening a seam: `TieredRouter.model` is now injectable, because `route` constructed `ModelRouter()` inline and the fallback chain was unreachable from a test on a machine that has FoundationModels. |
