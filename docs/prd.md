...
| REQ-SCR-004 | Ties are exactly the engine's: rank ranges use the native margin, and the tie note states that margin on the /100 scale. | **M14-W4.** `Uncertainty.swift::leaderSentence`. Cited by `ScoresTests.swift::testTheLeaderNoteSpeaksPointsWhenAnchored`. Amended by D-146 clause 3. |
