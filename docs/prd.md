...
| REQ-RUN-002 | The 12-hour refresh has completed at least two unattended cycles on the schedule, and the status file it left is read back and reported. | **NOT MET, and this row is late.** The M11 plan assigned it to W3 and W3 closed without it — the requirement was never written into this PRD at its wave, which is the F-1 drift the record contract exists to stop, and it is why the gap was invisible until a closing audit. `launchctl list` shows nothing loaded. One owner command starts it; two cycles then have to be OBSERVED, not assumed. Tracked as W-054 and W-071. |
