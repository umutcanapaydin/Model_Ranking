...
**Commit identity (D-161):** in a session, commits carry the owner's git identity, and an agent's commits add `GP-Agent:` and `GP-Task:` trailers, so no agent commit reads as the owner's own; every automated commit, including the CI issue agent's, carries `gp-agent <gp-agent@users.noreply.github.com>`. `conformance/test-commit-identity.py` verifies the range. Why the rule is shaped this way: `.agents/rules/git-authority.md`.
...
(1) `permission-matrix.md` — what's allowed · (2) `docs/decisions.md` — what is settled (project ADRs start at D-100; process ADRs use P-00x — seed B.6) · (3) `docs/prd.md` — the REQ-IDs your change relates to · (4) `.agents/rules/practices.md` — engineering rules · (5) existing code touching the same area.
...
- **Review depth, P-005 (this project):** a LOW/MED wave gets ONE combined independent reviewer; a HIGH wave (auth/payment/crypto/migration/distributed-correctness — auto-escalated) gets Code-Reviewer, then Tester, as separate subagents (`.claude/agents/`), plus a security pass on its slice. An escaped blocker on a tiered-down wave → full review until the next clean milestone. A `/fix-issue` fix gets the Tester alone. This project also keeps a security seat at every milestone close, ahead of Stage 5.1.
...
- **K.7** — fresh eyes: the reviewer/tester never authored the wave's code; the review is a FILE declaring `seat: independent` (`scripts/wave_check.py::review_seat_problems`). Why, and what the gate does not prove: `.agents/rules/review-seats.md`.
- **K.8** — shared contracts grep-verified in the plan (paste `grep -n` output); a plan that builds a SCREEN maps each fact to the published field it comes from (D-150 clause 2).
...
- **4.1 Quality Gate — OPTIONAL, off by default** (owner turns it on in the brief): REQ-ID trace + coverage delta + cost-log + the owner review pack `docs/closure-report-m{N}.md` (from `docs/closure-report.template.md`), graded by `make closes`, plus the owner's own milestone test session. Off, the per-wave Tester's citing-test rule is the check.
...
End every task with: files changed · tests run + outcomes · assumptions made · new ADRs (D-IDs) · risks queued to the next milestone.
