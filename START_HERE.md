...
- **Brand new project** → walk Stage 0 (Bootstrap) with the user. Start with §"Day-0" in [`pipeline-design.md`](https://github.com/SADCAIVibe/General_Pipeline/blob/v5.0/general_pipeline_v5.0/pipeline-design.md) §12.
...
1. **Layer 1 — Starter Package** (145 files in this repo) — pre-filled scaffolding from Phase-1 + AI-native harness.
...
1. **`AGENTS.md` (a ≤80-line TARGET (currently 145 — the target is aspirational and this file will not pretend otherwise) target, ≤150 hard cap) is canonical.** `CLAUDE.md` is a symlink to it. Industry standard (60k+ public repos use AGENTS.md). Both names work; same file.
...
- `.mcp.json` — content at `.mcp.json` (write it yourself; no template ships — an earlier reference to `mcp.json.template` pointed at a file that has never existed in any version) (also in `docs/claude-harness-config.md` "File 2")
...
1. **`AGENTS.md`** (a ≤80-line TARGET (currently 145 — the target is aspirational and this file will not pretend otherwise)) — house rules
...
5. **[`pipeline-design.md`](https://github.com/SADCAIVibe/General_Pipeline/blob/v5.0/general_pipeline_v5.0/pipeline-design.md)** — full spec (1455 lines; reference, not linear read)
6. **`.agents/rules/playbook-seeds.md`** — 64 + 8 seeds (search-as-needed)
7. **`docs/onboarding.md`** — the Monday-start / Friday-milestone-done playbook
...
2. **Reading [`pipeline-design.md`](https://github.com/SADCAIVibe/General_Pipeline/blob/v5.0/general_pipeline_v5.0/pipeline-design.md) linearly first.** It's 1455 lines; read this orientation + AGENTS.md instead. Reference v2-design as needed.
...
- ❌ A finished product. v2.2 has known gaps documented in [`pipeline-design.md`](https://github.com/SADCAIVibe/General_Pipeline/blob/v5.0/general_pipeline_v5.0/pipeline-design.md) §15 (several narrowed/closed across v2.1 + v2.2).
